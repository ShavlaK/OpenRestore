Инструкция по сборке OpenStore (Windows Edition)
...
2. Откройте папку OpenStore_Windows.
3. Дважды кликните по файлу `build.bat`.
4. Дождитесь завершения. В папке `dist/OpenStore/` появится готовая программа. Вы уже можете запускать OpenStore.exe оттуда.
...
4. Откройте файл `installer/OpenStore.iss` через Inno Setup.
5. Нажмите сверху кнопку "Compile" (или Build).
6. В папке `installer/Output/` появится ваш готовый `OpenStore_Setup.exe`!

Этот установщик сам проверит, есть ли на компьютере пользователя драйвера Apple. Если их нет, он тихо в фоновом режиме установит iTunes, а затем установит OpenStore.
